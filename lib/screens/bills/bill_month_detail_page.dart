...
class _BillMonthDetailPageState extends ConsumerState<BillMonthDetailPage> {
      ...
      child: Column(
        ...
        children: [
          Text(
            ...
            style: TextStyle(
                fontSize: 15, fontWeight: FontWeight.bold, color: AppPalette.text(ctx)),
          ),
          if (first.isNotEmpty) ...[
          ...
          children: [
            ...
            Row(
              children: [
                ...
                _col(isYear ? '年结余' : '月结余',
                    (m['balance'] ?? 0).toDouble(), AppPalette.text(ctx)),
              ],
            ),
          ],
        ),
  ...
  List<Widget> _expenseCats(List list) {
    if (list.isEmpty) {
      return [Text('无支出记录', style: TextStyle(color: AppPalette.textSecondary(ctx)))];
    }
    return list.map((x) {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Expanded(child: Text(cat, style: TextStyle(fontWeight: FontWeight.bold))),
        ...
        child: Column(
          ...
          children: [
            ...
            if (sub.isNotEmpty) ...[
              ...
              Text(sub,
                  style: TextStyle(fontSize: 11, color: AppPalette.textSecondary(ctx))),
            ],
          ],
        ),
      );
  ...
  Widget _compareBar(List rawList, bool isExpense) {
    ...
    if (list.isEmpty) {
      return Text('暂无数据', style: TextStyle(color: AppPalette.textSecondary(ctx)));
    }
            ...
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                ...
                getTitlesWidget: (v, _) {
                  ...
                  return Text('${m['label']}', style: TextStyle(fontSize: 10));
                },
              ),
            ),
  ...
  List<Widget> _topFlows(List list, {required bool isExpense}) {
    if (list.isEmpty) {
      return [Text('无记录', style: TextStyle(color: AppPalette.textSecondary(ctx)))];
    }
    return list.map((x) {
            ...
            children: [
              if (f.isAiSource) ...[
                Container(
                  ...
                  child: Text('AI',
                      style: TextStyle(fontSize: 9, color: AppPalette.card(ctx), fontWeight: FontWeight.bold)),
                ),
