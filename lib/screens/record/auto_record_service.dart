...
class AutoRecordService {
  ...
  Future<void> _processQueue(WidgetRef ref, BuildContext context) async {
    ...
    try {
      ...
      for (final raw in pending) {
        ...
        // v2.2.6：0 元 + 无配对时按营销黑名单再次过滤——
        // 0 元通知 99% 是「服务通知/钱已到账/红包/优惠券」等营销文案（参考用户 2026-09-09
        // 截图：支付宝「钱已到账 ¥0」「服务通知」、微信「兰夕 ¥0」陌生人红包均不应入账）。
        // 命中黑名单直接跳过；不在黑名单的仍走「待录入」保底（防漏真账）。
        if (parsed.amount == 0) {
          final t0 = ((raw['title']?.toString() ?? '') + ' ' + (raw['text']?.toString() ?? ''));
          const zeroBlacklist = [
            '服务通知', '钱已到账', '红包已领取', '领到红包', '已收下红包',
            '优惠已发放', '已发券', '抽奖', '获赠', '已到卡', '充值成功',
          ];
          if (zeroBlacklist.any((b) => t0.contains(b))) {
            recordLog('0元跳过:营销黑名单匹配 text=${t0.substring(0, t0.length < 60 ? t0.length : 60)}');
            handledRids.add(rid);
            await removePending(rid);
            continue;
          }
  ...
  ParsedNotification? _parse(Map<String, dynamic> raw) {
    ...
    // ---- ① 误识别过滤：强信号词（完成时态） ----
    // v2.2.6：移除单字「到账/入账/收款/转账」——太宽，会命中京东金融营销到账、支付宝
    // 服务通知等非真实收支（参考用户 2026-09-09 截图：京东「到账 ¥60」误记为收入、
    // 支付宝「钱已到账 ¥0」/「服务通知」误记、微信「兰夕 ¥0」陌生人红包误记）。
    // 只保留组合词：「收款到账/收款成功/入账成功/入账通知/已到账」等明确词组。
    const strongKw = [
