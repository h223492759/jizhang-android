...
class _WalletDetailPageState extends ConsumerState<WalletDetailPage> {
              ...
              children: [
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: _addFunds,
                          style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: AppPalette.text(ctx)),
                ...
                if (_allTxns.isEmpty)
                  const Padding(
                    ...
                    child: Text('还没有资金记录', style: TextStyle(color: AppPalette.textSecondary(ctx))),
                  )
                else
                  ..._allTxns.map((it) => Card(
                        child: ListTile(
                          tileColor: it.kind == 'monthly' ? AppPalette.background(ctx) : null,
                          title: Text(it.ymd),
  ...
  Widget _sumCol(String l, String v, Color? c) => Expanded(
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(l, style: TextStyle(fontSize: 11, color: AppPalette.textSecondary(ctx))),
            const SizedBox(height: 4),
            Text(v,
                style: TextStyle(
                    fontSize: 15,
                    fontWeight: FontWeight.bold,
                    color: c ?? AppPalette.text(ctx))),
          ],
        ),
      );
}
